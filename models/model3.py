...
class SANE(nn.Module):
    """
    SANE: Selective Affine with Normalization Experts
    - 两位专家：
        * Mean 专家: y_M = x + a
        * Contrast 专家: y_C = b * IN(x)
    - 自适应融合：
        * 空间门 α(h,w) ∈ [0,1]：决定是否采用专家输出（否则回退原样）
        * 通道门 β(c) ∈ [0,1]：在专家内部选择 C/M 的占比
    - 残差安全网 r ∈ [0,1]：总体回退强度
    依赖 only: a_vec(u_vec), b_vec(s_vec)
    """
    def __init__(self, channels: int,
                 gate_downsample: int = 8,   # 空间门轻量预测的下采样因子
        ...
        # 对比度专家用到的归一化（无仿射）
        if use_group_norm:
            # 也可选 GroupNorm，更稳的统计；与 IN 二选一即可
            # 这里把 num_groups 取为 8 的倍数，通道少时自动回退
            num_groups = max(1, min(8, channels))
        ...
        # 空间门 α(h,w)：逐通道深度可分离 -> 1x1，低分辨率预测再上采样
        ds = max(1, gate_downsample)
        self.spatial_gate = nn.Sequential(
            nn.Conv2d(channels, channels, kernel_size=3, stride=ds, padding=1, groups=channels),
            nn.GELU(),
            nn.Conv2d(channels, 1, kernel_size=1)
        )

        # 通道门 β(c)：把特征池化后预测每个通道的 C/M 偏好
        ...
        # 残差安全网 r：标量门，控制整体回退到 x 的比重
        self.residual_gate = nn.Parameter(torch.tensor(0.0))  # sigmoid -> [0,1]

    def forward(self, feat: torch.Tensor,
                a_vec: torch.Tensor,  # [B,C,1,1]  (来自你的 u_vec)
                b_vec: torch.Tensor   # [B,C,1,1]  (来自你的 s_vec)
                ) -> torch.Tensor:
        """
        feat: [B, C, H, W]
        a_vec: [B, C, 1, 1]  -- 亮度/色调专家的“加性锚”
        b_vec: [B, C, 1, 1]  -- 对比度专家的“尺度锚”
        """
        B, C, H, W = feat.shape
        assert C == self.C

        # 专家输出
        y_M = feat + a_vec                         # Mean 专家：仅加性
        x_hat = self.norm(feat)                    # Contrast 专家：先正规化
        # b 用 softplus 保证非负，避免语义翻转；并对 1 做对比度保底
        b_stable = 1.0 + F.softplus(b_vec) - 1.0   # = softplus(b)，写成这样便于理解“以1为保底”
        y_C = b_stable * x_hat

        # 通道门 β(c)：决定在专家内部 C/M 的权衡（每个通道一把尺）
        beta = torch.sigmoid(self.channel_gate(feat))  # [B,C,1,1]
        y_expert = beta * y_C + (1.0 - beta) * y_M     # [B,C,H,W]

        # 空间门 α(h,w)：决定是否采用专家输出，否则回退原样
        alpha_low = self.spatial_gate(feat)            # [B,1,H/ds,W/ds]
        alpha = torch.sigmoid(F.interpolate(alpha_low, size=(H, W), mode="bilinear", align_corners=True))

        # 残差安全网 r
        r = torch.sigmoid(self.residual_gate)

        # 融合：先在空间上选择专家/原样，再做整体残差回退
        y = alpha * y_expert + (1.0 - alpha) * feat
        out = (1.0 - r) * y + r * feat
        return out
...
class UIR_PolyKernel(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=3, dim=36, bias=False,
        ...
        self.sane = SANE(channels=dim, gate_downsample=8, use_group_norm=False)
    ...
    def forward(self, x, target=None, training: bool = True):
        ...
        if training:
            ...
            # ---- 概率注入（PAdaIN）到先验浅层特征 ----
            pr_shallow_mod = self._apply_sane(pr_shallow, po_u, po_s)
            ...
            pr_shallow_mod = self._apply_sane(pr_shallow, pr_u, pr_s)
