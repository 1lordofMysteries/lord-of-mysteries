...
# 自适应带通融合（低/高/梯度）+ LCN，完全无 FFT
class AdaptiveBandMixer(nn.Module):
    """
    设计要点：
      - 低频：3x3 平滑（AvgPool 等效）
      - 高频：LoG/Laplacian 响应（固定深度可分组核）
      - 梯度：Sobel 梯度幅值（固定核）
      - LCN：局部对比归一化，抑制光照偏置，保护颜色一致性
      - 融合权重：由 GAP( LN(x) ) 生成，Sigmoid 约束到 [0,1]
      - 残差尺度：可学习小尺度，稳健注入
    """
    def __init__(self, channels: int, lcn_ks: int = 7):
        super().__init__()
        self.C = channels
        self.lcn_ks = lcn_ks
        pad = lcn_ks // 2

        # Laplacian & Sobel：用 depthwise 固定卷积（不计参数）
        ...
        self.register_buffer('lap_k', lap.view(1,1,3,3).repeat(channels,1,1,1))
        self.register_buffer('sx_k',  sobel_x.view(1,1,3,3).repeat(channels,1,1,1))
        self.register_buffer('sy_k',  sobel_y.view(1,1,3,3).repeat(channels,1,1,1))

        # 融合权重预测：从全局统计生成 (a_low, a_high, a_grad) in [0,1]
        self.norm = LayerNorm(channels)
        self.abg = nn.Conv2d(channels, 3*channels, kernel_size=1, bias=True)

        # 残差强度与对比拉伸
        self.res_scale = nn.Parameter(torch.tensor(1e-3))   # 小值稳定注入
        self.boost = nn.Parameter(torch.tensor(0.5))        # 对比增强强度，初始温和

    def local_contrast_norm(self, x):
        # 近似 LCN： (x - μ) / (σ + eps)，μ/σ 用滑动窗口平均估计
        eps = 1e-6
        ks, pad = self.lcn_ks, self.lcn_ks // 2
        mu = F.avg_pool2d(x, kernel_size=ks, stride=1, padding=pad)
        x2 = x * x
        mu2 = F.avg_pool2d(x2, kernel_size=ks, stride=1, padding=pad)
        var = torch.relu(mu2 - mu * mu)
    ...
    def forward(self, x):
        ...
        # 高频：Laplacian 响应（每通道独立 depthwise）
        high = F.conv2d(x, self.lap_k.to(x.dtype), bias=None, stride=1, padding=1, groups=C)

        # 梯度：Sobel 幅值
        gx = F.conv2d(x, self.sx_k.to(x.dtype), bias=None, stride=1, padding=1, groups=C)
        gy = F.conv2d(x, self.sy_k.to(x.dtype), bias=None, stride=1, padding=1, groups=C)
        grad = torch.sqrt(gx * gx + gy * gy + 1e-6)

        # LCN：用于生成更加稳健的权重参考 & 轻微对比拉伸
        x_lcn = self.local_contrast_norm(x)
        # 通过 LN+GAP 的全局统计预测融合权重
        z = self.norm(x_lcn)
        z = F.adaptive_avg_pool2d(z, 1)
        a_low, a_high, a_grad = self.abg(z).chunk(3, dim=1)
        a_low  = torch.sigmoid(a_low)   # [B,C,1,1]
        a_high = torch.sigmoid(a_high)
        a_grad = torch.sigmoid(a_grad)

        # 带通融合：在 x 的基础上做稳健增强
        # 低频分量只做微调（色彩/光照矫正），高频与梯度提升纹理/边缘
        mix = x + self.boost * ( a_high * high + a_grad * grad ) + 0.25 * a_low * (low - x)

        # 小尺度残差注入，避免入口/出口位置过强改动
        out = x + self.res_scale * (mix - x)
        return out


# --------- 新版 HybridDomainAttention：无缝替换，IO/类名不变 ---------
class HybridDomainAttention(nn.Module):
    """
    适用于增强入口/出口的轻量注意力：
      - 无 FFT，低/高/梯度 三支固定核 + LCN，稳健不破坏颜色相位
      - 融合权重由全局统计自适应生成（Sigmoid 约束）
      - ECA 通道注意，极少参数
      - 两次残差（含小尺度），与原 FFN/LN 结构保持一致
    """
    def __init__(self, dim):
        super().__init__()
        ...
        self.band_mixer = AdaptiveBandMixer(dim)  # 频域替代：更轻、更稳
        self.chan_attn  = ECA(dim)                # 轻量通道注意

        self.ffn = FeedForward(dim, bias=False)

        # 最终残差缩放（深层更稳）
        self.out_scale = nn.Parameter(torch.tensor(1e-3))

    def forward(self, x):
        # 轻量带通 + LCN
        y = self.band_mixer(x)
        # 轻量通道注意
        y = self.chan_attn(self.norm1(y))
        # 残差一（小尺度）
        out = x + self.out_scale * (y - x)
        # FFN 残差二
