...
class SAdaIN(nn.Module):
    """Spatially-Adaptive AdaIN：a,b 从通道标量扩展为 [B,C,H,W] 的空间自适应参数"""
    def __init__(self, channels: int, hidden: int = None):
        super().__init__()
        if hidden is None:
            hidden = max(16, channels // 4 if channels >= 4 else 1)

        # 无仿射的 InstanceNorm
        self.norm = nn.InstanceNorm2d(channels, affine=False)

        # 由当前浅层特征 x 预测 Δa, Δb（空间自适应项）
        self.mlp = nn.Sequential(
            nn.Conv2d(channels, hidden, kernel_size=3, stride=1, padding=1, groups=1, bias=True),
            nn.GELU(),
            nn.Conv2d(hidden, 2 * channels, kernel_size=3, stride=1, padding=1, groups=1, bias=True)
        )

    def forward(self, x: torch.Tensor, a_vec: torch.Tensor, b_vec: torch.Tensor) -> torch.Tensor:
        """
        x:      [B,C,H,W]
        a_vec:  [B,C,1,1]  <- 来自 z 的加性锚
        b_vec:  [B,C,1,1]  <- 来自 z 的尺度锚
        """
        # 预测空间自适应的 Δa, Δb
        ab = self.mlp(x)                      # [B,2C,H,W]
        a_sp, b_sp = torch.chunk(ab, 2, dim=1)

        # 将通道锚广播并与空间项相加得到 a(h,w), b(h,w)
        a = a_sp + a_vec                      # [B,C,H,W]
        b = b_sp + b_vec                      # [B,C,H,W]
...
class UIR_PolyKernel(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=3, dim=36, bias=False,
        ...
        self.s_adain = SAdaIN(channels=dim)
    ...
    def _p_adain(self, feat, u_vec, s_vec):
        ...
        """
        #self._s_adain = SAdaIN(channels=feat.size(1))
        
        #feat_norm = self.insnorm(feat)
        #return feat_norm * torch.abs(s_vec) + u_vec
        return self.s_adain(feat, u_vec, s_vec)
