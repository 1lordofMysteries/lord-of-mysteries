...
class MambaBlock2D(nn.Module):
    ...
    def __init__(self, channels: int, d_state: int = 16, d_conv: int = 4, expand: int = 2, drop_path: float = 0.0):
        ...
        self.core = BiGMa(input_dim=c_proj, output_dim=c_proj, d_state=d_state, d_conv=d_conv, expand=expand)
...
class MaPU(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=3, dim=36, bias=False,
        ...
        super(MaPU, self).__init__()

        # ---------------- 原模型先验主干（prior, 输入为 x） ----------------
        self.input_embed = nn.Conv2d(in_channels, dim, kernel_size=1)
        #self.encoder_level1 = LEDA(dim)
        self.encoder_level1 = nn.Identity()
        ...
        #self.bottleneck = APEX(int(dim * 2 ** 2))
        ...
        #self.decoder_level1 = LEDA(int(dim))
        self.decoder_level1 = nn.Identity()
        ...
        #self.encoder_level1_po = LEDA(dim)
        self.encoder_level1_po = nn.Identity()
        ...
        #self.bottleneck_po = APEX(int(dim * 2 ** 2))
        self.bottleneck_po = nn.Identity()
