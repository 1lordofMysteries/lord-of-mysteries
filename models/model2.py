...
class PAdaINPlus(nn.Module):
    """
    升级版 PAdaIN：
      1) softplus(s) 稳定尺度
      2) 温度控制：b' = 1 + tau*(b-1), a' = tau*a，tau∈(0,1]
      3) 空间门控：alpha(h,w)∈(0,1)，低分辨率预测后上采样
      4) 分组多头（可选）：对通道分组做独立调控（这里分别对 scale/shift 各做一套）
    """
    def __init__(self, channels: int, z_dim: int = 0,
                 groups: int = 1,
                 gate_size: int = 8,
                 ...
                 eps: float = 1e-6):
        super().__init__()
        assert channels % groups == 0, "channels 必须能被 groups 整除"
        self.C = channels
        self.G = groups
        self.z_dim = z_dim
        ...
        # 归一化（可替换为 GroupNorm/LayerNorm 做 ablation）
        self.inorm = nn.InstanceNorm2d(channels, affine=False)

        # -------- 温度 τ 预测/学习 --------
        if learnable_tau and z_dim > 0:
            # 由 z 预测通道级温度 τ ∈ (0,1]
            self.tau_head = nn.Sequential(
                nn.Conv2d(z_dim, channels, kernel_size=1, bias=True),
                nn.Sigmoid()
            )
        elif learnable_tau and z_dim == 0:
            # 全局标量 τ
            self.tau_param = nn.Parameter(torch.tensor(0.5))
        else:
            self.register_buffer("tau_const", torch.tensor(1.0))

        # -------- 空间门控 α(h,w) --------
        if use_spatial_gate:
            in_ch = channels + (z_dim if z_dim > 0 else 0)
            hidden = max(16, channels // 4)
            self.gate_predictor = nn.Sequential(
                nn.Conv2d(in_ch, hidden, kernel_size=1),
                nn.ReLU(inplace=True),
                nn.Conv2d(hidden, 1, kernel_size=1),   # 低分辨率 gate（后面再上采样）
            )
            self.gate_size = gate_size
            self.gap_to_map = nn.Conv2d(channels, channels, kernel_size=1)

        # -------- 分组/多头重标定（修复点）--------
        # 以前是把 scale 和 shift 拼成 2C 再用一个 conv；现在改为分别一套，避免通道数不匹配
        self.use_groups = (groups > 1)
        if self.use_groups:
            self.grp_recalib_scale = nn.Conv2d(channels, channels, kernel_size=1,
                                               groups=groups, bias=True)
            self.grp_recalib_shift = nn.Conv2d(channels, channels, kernel_size=1,
                                               groups=groups, bias=True)

    def _get_tau(self, z_vec: torch.Tensor = None):
        if self.learnable_tau:
            if self.z_dim > 0 and z_vec is not None:
                tau = self.tau_head(z_vec)  # [B,C,1,1]
            else:
                tau = torch.sigmoid(self.tau_param).clamp(0.01, 1.0)
                tau = tau.view(1, 1, 1, 1)
        else:
            tau = self.tau_const.view(1, 1, 1, 1)
        return tau

    def _get_alpha(self, feat: torch.Tensor, z_vec: torch.Tensor = None):
        if not self.use_spatial_gate:
            return torch.ones_like(feat[:, :1])  # [B,1,H,W]
        B, C, H, W = feat.shape
        g = F.adaptive_avg_pool2d(feat, 1)    # [B,C,1,1]
        g = self.gap_to_map(g)                # [B,C,1,1]
        cond = g
        if self.z_dim > 0 and z_vec is not None:
            cond = torch.cat([cond, z_vec], dim=1)  # [B,C+z,1,1]
        gate_lr = self.gate_predictor(cond)   # [B,1,1,1]
        gate_lr = gate_lr.expand(B, 1, self.gate_size, self.gate_size)
        alpha = torch.sigmoid(
            F.interpolate(gate_lr, size=(H, W), mode='bilinear', align_corners=False)
        )  # [B,1,H,W]
        return alpha

    def forward(self, feat: torch.Tensor,
                u_vec: torch.Tensor,   # [B,C,1,1]
                s_vec: torch.Tensor,   # [B,C,1,1]
                z_vec: torch.Tensor = None):
        """
        返回：y ∈ [B,C,H,W]
        """
        # 1) 归一化
        x_hat = self.inorm(feat)

        # 2) 稳定尺度/偏移
        scale = F.softplus(s_vec) + self.eps   # b = softplus(s) + eps
        shift = u_vec                          # a = u

        # 3) 温度控制
        tau = self._get_tau(z_vec)             # [B,C,1,1] or [1,1,1,1]
        if tau.numel() == 1:
            tau = tau.expand(x_hat.size(0), self.C, 1, 1)
        scale = 1.0 + tau * (scale - 1.0)      # b' = 1 + tau*(b-1)
        shift = tau * shift                    # a' = tau*a

        # 4) 分组/多头重标定（修复：分别作用在 scale/shift）
        if self.use_groups:
            scale = self.grp_recalib_scale(scale)  # [B,C,1,1]
            shift = self.grp_recalib_shift(shift)  # [B,C,1,1]

        # 5) 空间门控
        alpha = self._get_alpha(feat, z_vec)       # [B,1,H,W]

        # 6) 混合输出
        y_strong = x_hat * scale + shift
        y = (1.0 - alpha) * x_hat + alpha * y_strong
        return y
...
class UIR_PolyKernel(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=3, dim=36, bias=False,
        ...
        self.p_adain = PAdaINPlus(channels=dim,z_dim=z_dim,groups=4,gate_size=8,use_spatial_gate=True,learnable_tau=True)
    ...
    def forward(self, x, target=None, training: bool = True):
        ...
        if training:
            ...
            pr_shallow_mod = self.p_adain(feat=pr_shallow,u_vec=po_u,s_vec=po_s,z_vec=po_latent_u)
            ...
            pr_shallow_mod = self.p_adain(feat=pr_shallow,u_vec=pr_u,s_vec=pr_s,z_vec=pr_latent_u)
